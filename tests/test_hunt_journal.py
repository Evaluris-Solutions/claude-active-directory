...
class TestJournalQuery:
    ...
    def test_query_by_target(self, journal_path, sample_journal_entry):
        ...
        results = journal.query(target="corp.local")
        assert len(results) == 1
        assert results[0]["target"] == "corp.local"
    ...
    def test_query_by_vuln_class(self, journal_path, sample_journal_entry):
        ...
        results = journal.query(vuln_class="kerberoast")
        assert len(results) == 1
    ...
    def test_query_multiple_filters(self, journal_path, sample_journal_entry):
        ...
        results = journal.query(target="corp.local", result="confirmed")
        assert len(results) == 1

        results = journal.query(target="corp.local", result="rejected")
