#!/usr/bin/env python3
"""
agent.py — LangGraph-style ReAct agent for authorized AD / infrastructure assessments.
...
AGENT_SYSTEM = """\
You are assisting with an authorized Active Directory / internal security assessment.
Tools map to recon and follow-on checks; many legacy web-scan tools may no-op in the AD edition.

CORE RULES:
1. Start with run_recon if no recon data exists — domain/LDAP-oriented enumeration.
2. After recon, read_recon_summary to review collected artifacts.
3. Prioritize credential and privilege paths: Kerberos abuse, misconfig ACLs, AD CS, delegation.
4. Maintain notes via update_working_memory after significant discoveries.
5. Call finish when objectives are met, time is exhausted, or ROE blocks further action.
6. Do not repeat identical tool calls without new inputs.
7. Respect rules of engagement at all times.

Think step by step. Pick the highest-value next action for domain compromise paths."""
...
def run_agent_hunt(
    ...
    *,
    ...
    # Per-domain engagement folder (AD edition) — session artifacts live with recon data
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ReAct agent — AD assessment helpers with Ollama tool calling",
