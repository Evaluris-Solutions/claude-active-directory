#!/usr/bin/env python3
"""
learn.py — Fetches advisory/CVE intel for product keywords.
Queries GitHub Advisory Database and NVD CVE API.

Usage:
  python3 tools/learn.py --tech "windows,active-directory"
  python3 tools/learn.py --tech "windows" --target corp.local --output recon/corp.local/intel.md
"""
...
def fetch_intel(techs: list[str]) -> list[dict]:
    """Collect intel from GitHub Advisory + NVD for all tech keywords."""
    all_results = []
    for tech in techs:
...
def build_markdown(techs: list[str], results: list[dict]) -> str:
    ...
    lines = [
        ...
        f"**Sources:** GitHub Advisory DB, NVD CVE API",
        f"",
        f"---",
        f"",
    ]
...
def main():
    parser = argparse.ArgumentParser(description="Fetch CVE/advisory intel for product keywords")
    parser.add_argument("--tech", required=True, help="Comma-separated keywords (e.g. windows,active-directory)")
    ...
    print(f"\n{BOLD}Intel fetch (AD edition){RESET}")
